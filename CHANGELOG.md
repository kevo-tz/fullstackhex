...
- **Auth metrics middleware**: `track_auth_metrics` in `auth/src/metrics.rs` — records `auth_requests_total`, `auth_latency_seconds`, `auth_errors_total`
...
- **`token_refresh_total` counter**: dedicated counter for token refresh success events
- **Auth latency histogram buckets**: custom buckets (1ms–5s) for `auth_latency_seconds`
